...
\begin{abstract}
Este reporte interpreta tres comunidades locales del Mapper orbital dominadas por velocidad radial: \texttt{cube17\_cluster2}, \texttt{cube26\_cluster0} y \texttt{cube24\_cluster0}. El objetivo no es afirmar planetas faltantes, sino estudiar si un exoplaneta observado puede funcionar como ancla de una vecindad físico-orbital potencialmente submuestreada. El caso principal es HIP 90988 b, planeta ancla de \texttt{cube17\_cluster2}. La evidencia muestra que HIP 90988 b vive en un nodo con alta sombra observacional, frontera metodológica fuerte, imputación nula en las variables de $\mathbb{R}^3$ y déficit relativo local débil. La conclusión correcta es prudente: topológicamente, HIP 90988 b no prueba que existan planetas no detectados, pero sí señala una región donde el catálogo podría estar submuestreado hacia objetos de menor masa o menor proxy de detectabilidad por velocidad radial.
\end{abstract}
...
\begin{quote}
HIP 90988 b muestra un déficit topológico local débil de vecinos compatibles respecto al vecindario Mapper. Esta señal es coherente con incompletitud observacional hacia menor proxy RV, pero no alcanza para afirmar una población faltante numerosa.
\end{quote}

El resultado de referencia por nodos análogos debe leerse con más cautela. En la figura, la referencia análoga es mucho menor que la referencia por vecinos. Eso sugiere que los nodos análogos usados por el criterio automático pueden no ser una línea base suficientemente comparable para estimar abundancia. Por tanto, la interpretación principal debe apoyarse en la comparación con el vecindario topológico, no en una extrapolación absoluta.
