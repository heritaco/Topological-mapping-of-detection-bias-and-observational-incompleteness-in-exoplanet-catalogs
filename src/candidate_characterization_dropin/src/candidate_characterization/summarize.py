...
def build_markdown_summary(predictions: pd.DataFrame, report_dir: Path, validation_metrics: Optional[pd.DataFrame] = None) -> Path:
    ...
    if predictions.empty:
        ...
        lines.append(top[keep].head(20).to_markdown(index=False))
    if validation_metrics is not None and not validation_metrics.empty:
        lines.append("")
        lines.append("## Validation")
        lines.append("")
        lines.append(validation_metrics.to_markdown(index=False))
    lines.append("")
