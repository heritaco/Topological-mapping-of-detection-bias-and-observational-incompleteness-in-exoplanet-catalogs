...
def build_interpretation_sentences(region_profile: pd.DataFrame, anchor_profile: pd.DataFrame) -> list[str]:
    ...
    if not region_profile.empty:
        ...
        sentences.append(
            f"La región con mayor prioridad regional es {best.get('node_id', 'unknown')}, "
            f"con TOI={best.get('TOI', float('nan')):.4g}. La lectura debe descomponerse en sombra, "
            "imputación R^3, continuidad física y soporte de red antes de convertirla en conclusión astrofísica."
        )
    if not anchor_profile.empty:
        ...
        sentences.append(
            ...
            f"con ATI={best.get('ATI', float('nan')):.4g}. Este valor prioriza inspección, no prueba objetos ausentes."
        )
    sentences.append(
        "El criterio central del reporte es explicar por qué gana cada caso: qué factor multiplica el índice, qué factor lo limita y qué tan robusto es el déficit por radio."
    )
    return sentences
